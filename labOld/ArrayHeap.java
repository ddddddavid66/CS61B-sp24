...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    public ArrayHeap() {
        contents = new ArrayHeap.Node[16]; //默认长度16
        ...
        Mymap = new HashMap<>(); // 记录 item 和 index 之间 的 关系
    }
    ...
    private void swap(int index1, int index2) {
        ...
        Mymap.put(node2.myItem,index1);
        Mymap.put(node1.myItem,index2);
    }
    ...
    private void sink(int index) {
        ...
        int small = index;
        if (left <= size && min(small, left) == left) {
            small = left;
        }
        if (right <= size && min(small, right) == right) {
            small = right;
        }
        if (small != index) {
            swap(small, index);
    ...
    public T peek() {  // 返回最小值
        Node node = getNode(1);
        return node == null ? null : node.myItem;
    }
    ...
    public void changePriority(T item, double priority) {
        Integer index = Mymap.get(item);
        if(index == null){
            return;
        }
        double old = getNode(index).myPriority;
